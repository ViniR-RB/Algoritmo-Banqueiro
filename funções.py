...
def get_recursos_totais(qtd_tipos_recursos):
    ...
    for i in range(qtd_tipos_recursos):
        recurso = int(input('Digite a quantidade de Recursos %d:\n'
        '-->' %(i+1)))
...
def get_matriz_alocados(qtd_processos,qtd_tipos_recursos):
    ...
    for i in range(len(matriz_alocados)):
        for j in range(len(matriz_alocados[i])):
            matriz_alocados[i,j] += int(input('Digite a quantidade de recursos %d alocados para o processo %d:\n'\
                '-->' %  ((j+1), (i+1))))
...
def algoritmo_banqueiro(qtd_processos,qtd_tipos_recursos,recursos_disponiveis,matriz_alocados,matriz_recursos_necessarios):
    ...
    print('--> Os processos não entraram em deadlock')
